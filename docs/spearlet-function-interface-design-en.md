...
- For `artifact_type = "wasm"`, `location` must specify a supported scheme. Current support includes `smsfile://<file_id>`, which is fetched via SMS. Supplying `checksum` (SHA-256) is recommended for integrity verification.
