...
async fn example_basic_operations() -> Result<(), Box<dyn std::error::Error>> {
    use spear_next::storage::{KvStore, MemoryKvStore};
    ...
    store
        .put(&"user:123".to_string(), &b"John Doe".to_vec())
        .await?;
    store
        .put(&"user:456".to_string(), &b"Jane Smith".to_vec())
        .await?;
    store
        .put(&"config:timeout".to_string(), &b"30".to_vec())
        .await?;
...
async fn example_batch_operations() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let keys_to_delete = vec!["batch:002".to_string(), "batch:004".to_string()];
...
async fn example_kv_node_registry() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Registered nodes: {}, {}, {}",
        node1.uuid, node2.uuid, node3.uuid
    );

    // Update heartbeat / 更新心跳
    registry
        .update_heartbeat(&node1.uuid, chrono::Utc::now().timestamp())
        .await?;
...
async fn example_sled_persistence() -> Result<(), Box<dyn std::error::Error>> {
    use spear_next::storage::{KvStore, SledKvStore};
    ...
    // Method 1: Create Sled store directly / 方法1：直接创建Sled存储
    {
        ...
        store
            .put(
                ...
                &b"This data will persist".to_vec(),
            )
            .await?;
        store
            .put(
                &"persistent:key2".to_string(),
                &b"Even after restart".to_vec(),
            )
            .await?;
    ...
    // Method 2: Use factory function / 方法2：使用工厂函数
    {
        ...
        store
            .put(
                &"persistent:key3".to_string(),
                &b"Added after reopen".to_vec(),
            )
            .await?;
...
async fn example_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "Successfully retrieved {} out of {} keys",
