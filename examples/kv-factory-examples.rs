...
use spear_next::storage::{
    ...
    DefaultKvStoreFactory, KvStore, KvStoreConfig, KvStoreFactory,
};
...
async fn basic_factory_usage() -> Result<(), Box<dyn std::error::Error>> {
    ...
    memory_store
        .put(&"key1".to_string(), &"value1".as_bytes().to_vec())
        .await?;
    let value = memory_store.get(&"key1".to_string()).await?;
    println!(
        "Memory store - Retrieved: {:?}",
    ...
    #[cfg(feature = "sled")]
    {
        ...
        // Test basic operations / 测试基本操作
        sled_store
            .put(&"key2".to_string(), &"value2".as_bytes().to_vec())
            .await?;
        let value = sled_store.get(&"key2".to_string()).await?;
        println!(
            "Sled store - Retrieved: {:?}",
            String::from_utf8_lossy(&value.unwrap())
        );
    }
...
async fn env_var_configuration() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test operations / 测试操作
    store
        .put(&"env_key".to_string(), &"env_value".as_bytes().to_vec())
        .await?;
    let value = store.get(&"env_key".to_string()).await?;
    println!(
        "Environment-configured store - Retrieved: {:?}",
        String::from_utf8_lossy(&value.unwrap())
    );
...
async fn custom_factory_implementation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    impl KvStoreFactory for LoggingKvStoreFactory {
        ...
        ) -> Result<Box<dyn KvStore>, spear_next::sms::error::SmsError> {
        ...
        fn validate_config(
            &self,
            config: &KvStoreConfig,
        ) -> Result<(), spear_next::sms::error::SmsError> {
    ...
    store
        .put(
            ...
            &"custom_value".as_bytes().to_vec(),
        )
        .await?;
    let value = store.get(&"custom_key".to_string()).await?;
    println!(
        "Custom factory store - Retrieved: {:?}",
        String::from_utf8_lossy(&value.unwrap())
    );
...
async fn runtime_backend_switching() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (environment, backend) in conditions {
        ...
        let config = match backend {
            "memory" => KvStoreConfig::memory().with_param("environment", environment),
            #[cfg(feature = "sled")]
...
pub async fn create_store_for_mode(
    mode: &str,
) -> Result<Box<dyn KvStore>, Box<dyn std::error::Error>> {
...
pub async fn safe_create_store(
    config: &KvStoreConfig,
) -> Result<Box<dyn KvStore>, Box<dyn std::error::Error>> {
