...
async fn custom_factory_implementation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    impl KvStoreFactory for LoggingKvStoreFactory {
        async fn create(&self, config: &KvStoreConfig) -> Result<Box<dyn KvStore>, spear_next::common::error::SmsError> {
        ...
        fn validate_config(&self, config: &KvStoreConfig) -> Result<(), spear_next::common::error::SmsError> {
