...
async fn test_gateway_router_with_different_states() {
    ...
    for i in 0..5 {
        ...
        assert!(std::mem::size_of_val(&router) > 0, "Router {} creation failed", i);
    }
}
...
async fn test_gateway_state_memory_usage() {
    ...
    assert!(state_size < 1024, "Gateway state uses too much memory: {} bytes", state_size);
}
...
async fn test_multiple_gateway_states() {
    ...
    for (i, state) in states.iter().enumerate() {
        assert!(std::mem::size_of_val(&state.node_client) > 0, "State {} node client invalid", i);
        assert!(std::mem::size_of_val(&state.task_client) > 0, "State {} task client invalid", i);
    }
}
...
mod integration_tests {
    ...
    async fn test_gateway_creation_performance() {
        ...
        for _ in 0..100 {  // Reduced from 1000 to 100 for async
            let state = create_mock_gateway_state().await;
        ...
        assert!(duration.as_secs() < 5, "Gateway creation too slow: {:?}", duration);
    }
