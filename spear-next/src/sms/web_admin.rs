...
pub fn create_admin_router(state: GatewayState) -> Router {
    Router::new()
        ...
        .route("/admin/api/tasks", get({
            let state = state.clone();
            move |q: Query<ListQuery>| list_tasks(state.clone(), q)
        }))
        .route("/admin/api/tasks", post({
            let state = state.clone();
...
async fn list_tasks(state: GatewayState, Query(q): Query<ListQuery>) -> Json<serde_json::Value> {
    ...
    let resp = client.list_tasks(ListTasksRequest { node_uuid: String::new(), status_filter: -1, priority_filter: -1, limit: 0, offset: 0 }).await.unwrap().into_inner();
    let total = resp.total_count as usize;
    let mut list = resp.tasks.into_iter().map(|t| {
        let status = match t.status { 1 => "registered", 2 => "active", 3 => "inactive", 4 => "unregistered", _ => "unknown" };
        let priority = match t.priority { 1 => "low", 2 => "normal", 3 => "high", 4 => "urgent", _ => "unknown" };
        let (exec_type, exec_uri, exec_name) = if let Some(exec) = t.executable {
            let et = match exec.r#type { 1 => "binary", 2 => "script", 3 => "container", 4 => "wasm", 5 => "process", _ => "unknown" };
    ...
    if let Some(qs) = q.q.as_ref().map(|s| s.to_lowercase()) {
        list = list.into_iter().filter(|item| {
            ...
            let meta = item["metadata"].as_object();
            let meta_hit = meta.map(|m| m.iter().any(|(k,v)| k.to_lowercase().contains(&qs) || v.as_str().unwrap_or("").to_lowercase().contains(&qs))).unwrap_or(false);
            id.contains(&qs) || name.contains(&qs) || node.contains(&qs) || endpoint.contains(&qs) || meta_hit
        }).collect();
    }
    let (field, asc) = if let Some(sort) = &q.sort {
        let mut parts = sort.split(':');
...
async fn create_task(state: GatewayState, axum::extract::Json(body): axum::extract::Json<CreateTaskBody>) -> Json<serde_json::Value> {
    use crate::proto::sms::{RegisterTaskRequest, TaskExecutable, ExecutableType, TaskPriority};
    ...
    match resp {
        Ok(r) => {
            ...
            Json(json!({ "success": inner.success, "task_id": inner.task_id, "message": inner.message }))
        }
        Err(e) => Json(json!({ "success": false, "message": e.to_string() })),
    }
}

async fn get_task_detail(state: GatewayState, Path(task_id): Path<String>) -> Json<serde_json::Value> {
    ...
    match resp {
        Ok(r) => {
            ...
            if let Some(t) = inner.task {
                let status = match t.status { 1 => "registered", 2 => "active", 3 => "inactive", 4 => "unregistered", _ => "unknown" };
                let priority = match t.priority { 1 => "low", 2 => "normal", 3 => "high", 4 => "urgent", _ => "unknown" };
                let (exec_type, exec_uri, exec_name, exec_sum, exec_args, exec_env) = if let Some(exec) = t.executable {
                    let et = match exec.r#type { 1 => "binary", 2 => "script", 3 => "container", 4 => "wasm", 5 => "process", _ => "unknown" };
                    (et.to_string(), exec.uri, exec.name, exec.checksum_sha256, exec.args, exec.env)
                } else { (String::new(), String::new(), String::new(), String::new(), vec![], std::collections::HashMap::new()) };
                Json(json!({
                    "found": true,
                    "task": {
                        "task_id": t.task_id,
...
async fn admin_static(headers: HeaderMap, Path(path): Path<String>) -> impl IntoResponse {
    ...
    let (bytes, mime, content_encoding) = match (path, enc.as_str()) {
        ("react-app.js", enc) if enc.contains("br") => (include_bytes!(concat!(env!("OUT_DIR"), "/react-app.js.br")).as_ref(), "application/javascript", Some("br")),
        ("react-app.js", enc) if enc.contains("gzip") => (include_bytes!(concat!(env!("OUT_DIR"), "/react-app.js.gz")).as_ref(), "application/javascript", Some("gzip")),
        ("style.css", enc) if enc.contains("br") => (include_bytes!(concat!(env!("OUT_DIR"), "/style.css.br")).as_ref(), "text/css", Some("br")),
        ("style.css", enc) if enc.contains("gzip") => (include_bytes!(concat!(env!("OUT_DIR"), "/style.css.gz")).as_ref(), "text/css", Some("gzip")),
        ("index.html", enc) if enc.contains("br") => (include_bytes!(concat!(env!("OUT_DIR"), "/index.html.br")).as_ref(), "text/html", Some("br")),
        ("index.html", enc) if enc.contains("gzip") => (include_bytes!(concat!(env!("OUT_DIR"), "/index.html.gz")).as_ref(), "text/html", Some("gzip")),
        ("react-app.js", _) => (include_bytes!("../../assets/admin/react-app.js").as_ref(), "application/javascript", None),
        ("style.css", _) => (include_bytes!("../../assets/admin/style.css").as_ref(), "text/css", None),
        ("index.html", _) => (include_bytes!("../../assets/admin/index.html").as_ref(), "text/html", None),
        ("app.js", _) => (include_bytes!("../../assets/admin/app.js").as_ref(), "application/javascript", None),
    ...
    resp.headers_mut().insert(CACHE_CONTROL, cache.parse().unwrap());
    resp.headers_mut().insert(axum::http::header::VARY, "Accept-Encoding".parse().unwrap());
    if let Some(ce) = content_encoding { resp.headers_mut().insert(axum::http::header::CONTENT_ENCODING, ce.parse().unwrap()); }
    resp
}
