...
use super::{
    ...
    ChannelConfig, RuntimeInstanceId,
};
use super::channel::{UnixSocketChannel, TcpChannel, GrpcChannel};
...
impl CommunicationFactory {
    ...
    pub async fn create_channel_for_instance(
        ...
        let strategy = self.strategies.get(&instance_id.runtime_type)
        ...
        // 首先尝试首选通道
        if let Ok(channel) = self.create_channel_by_type(instance_id.clone(), &strategy.preferred_channel, config.clone()).await {
        ...
        for fallback_type in &strategy.fallback_channels {
            if let Ok(channel) = self.create_channel_by_type(instance_id.clone(), fallback_type, config.clone()).await {
                return Ok(Arc::from(channel));
            }
        }
    ...
    /// Generate instance-specific address
    /// 生成实例特定的地址
    fn generate_instance_address(&self, instance_id: &RuntimeInstanceId, config: &ChannelConfig) -> String {
        match config.channel_type.as_str() {
            "unix" => {
                format!("/tmp/spear-{}-{}.sock", 
            ...
            "tcp" => {
                ...
                let base_port = 50051;
                let instance_hash = instance_id.instance_id.chars()
                    .map(|c| c as u32)
                    .sum::<u32>() % 1000;
    ...
    /// 关闭并从池中移除通道
    pub async fn close_channel(&mut self, instance_id: &RuntimeInstanceId) -> CommunicationResult<()> {
    ...
    pub fn is_channel_supported(&self, runtime_type: &RuntimeType, channel_type: &str) -> bool {
        if let Some(strategy) = self.strategies.get(runtime_type) {
            strategy.get_all_channels().contains(&channel_type.to_string())
        } else {
            false
        }
    }
...
impl CommunicationStrategyBuilder {
    ...
    pub fn build(self) -> CommunicationResult<CommunicationStrategy> {
        ...
        let preferred_channel = self.preferred_channel
            .ok_or_else(|| CommunicationError::InvalidConfiguration {
...
mod tests {
    ...
    fn test_channel_pool_management() {
        ...
        let instance_id1 = RuntimeInstanceId::new(
            RuntimeType::Process,
            "instance-1".to_string(),
        );
        
        let instance_id2 = RuntimeInstanceId::new(
            RuntimeType::Process,
