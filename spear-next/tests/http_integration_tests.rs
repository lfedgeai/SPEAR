...
async fn test_http_node_lifecycle() {
    ...
    let get_response = server
        .get(&format!("/api/v1/nodes/{}", created_uuid))
        .await;
    ...
    let channel_filter = tonic::transport::Channel::from_shared(format!("http://{}", grpc_addr_filter))
        .unwrap()
        .connect()
        .await
        .expect("Failed to connect to test gRPC server");
    ...
    let filter_body_bytes = axum::body::to_bytes(filter_response.into_body(), usize::MAX).await.unwrap();
    ...
    println!("Filtered nodes response: {}", serde_json::to_string_pretty(&list_active_body).unwrap());
...
async fn test_http_error_handling() {
    ...
    let incomplete_register_response = server
        .post("/api/v1/nodes")
    ...
    let invalid_uuid_response = server
        .get("/api/v1/nodes/invalid-uuid-format")
        .await;
...
async fn test_http_concurrent_operations() {
    ...
    for uuid in &registered_uuids {
        assert!(nodes.iter().any(|n| n["uuid"].as_str() == Some(uuid.as_str())));
    }
