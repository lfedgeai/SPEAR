...
use spear_next::proto::spearlet::{
    function_service_client::FunctionServiceClient, function_service_server::FunctionServiceServer,
    *,
};
...
mod test_utils {
    ...
    pub async fn create_test_function_service() -> (SocketAddr, tokio::task::JoinHandle<()>) {
        ...
        let function_service = FunctionServiceImpl::new(Arc::new(SpearletConfig::default()))
            .await
            .unwrap();
    ...
    pub async fn create_test_function_client(
        addr: SocketAddr,
    ) -> FunctionServiceClient<tonic::transport::Channel> {
        FunctionServiceClient::connect(format!("http://{}", addr))
            .await
            .unwrap()
    }
