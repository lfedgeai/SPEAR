...
mod test_utils {
    ...
    pub async fn create_test_grpc_client(
        addr: SocketAddr,
...
async fn test_grpc_node_lifecycle() {
    ...
    assert_eq!(
        updated_node.metadata.get("updated"),
        Some(&"true".to_string())
    );
...
async fn test_grpc_resource_management() {
    ...
    let update_resource_resp = client
        .update_node_resource(update_resource_req)
        .await
        .unwrap();
    ...
    let list_resources_resp = client
        .list_node_resources(list_resources_req)
        .await
        .unwrap();
    ...
    let get_with_resource_req = GetNodeWithResourceRequest { uuid: uuid.clone() };

    let get_with_resource_resp = client
        .get_node_with_resource(get_with_resource_req)
        .await
        .unwrap();
